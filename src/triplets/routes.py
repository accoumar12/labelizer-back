...
from src.core.api.auth.core import AdminUserSession, UserSession
...
from src.triplets.flows import (
...
async def download_db(
    ...
    if validation:
        ...
        filename = f"{now}_src_validation_triplets.csv"
        logger.info("Validation database downloaded.")
    else:
        stream = get_triplets_csv_stream(db)
        filename = f"{now}_src_triplets.csv"
